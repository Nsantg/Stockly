...
**Última ejecución registrada:** 7 de junio de 2026 — 40 pruebas pasaron, 2 omitidas, ninguna falló
...
Al último corrido teníamos 10 suites, 42 casos en total: 40 pasaron, 2 están omitidos con `skip` y ninguno falló.

| Archivo | Casos | Qué valida |
|---------|-------|------------|
| `VentaHandler.test.ts` | 6 | CP-10, CP-11, RN-06 |
| `DevolucionHandler.test.ts` | 6 | CP-15, CP-16, RN-04 |
| `AjusteSalidaHandler.test.ts` | 4 | CP-18, RN-06 |
| `MovementFactory.test.ts` | 4 | Resolución de handlers por tipo de movimiento |
| `InventoryService.test.ts` | 6 | FEFO (N2026-4) y alertas de stock (N2026-30 / CP-27) |
...
| `users.integration.test.ts` | 3 (2 omitidos) | RF-15, RF-16, RNF-03 |

Muchos casos del plan (CP-02, CP-06, CP-12, flujos E2E, etc.) siguen cubiertos de forma manual o pendientes de automatizar. El detalle completo está en `TRAZABILIDAD-Y-REPORTES-QA.md`.
...
2. **bcryptjs:** se simulan `hash` y `compare` para no depender del costo real de encriptación.
3. **Transacciones:** en `MovementService.test.ts` se mockea `getDataSource`; los handlers usan un `QueryRunner` falso definido en `movementTestHelpers.ts`.
4. **NextAuth:** en tests de controladores se mockea `getServerSession`.
5. **Servicios:** se instancian handlers directamente; los singletons de servicio tienen sus dependencias mockeadas a nivel de módulo.
